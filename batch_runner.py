...
async def main():
    ...
    for batch_num in range(total_runs // batch_size):
        logger.info(f"--- Starting Batch {batch_num + 1} of {total_runs // batch_size} ---")
        tasks = []
        ...
        for i in range(start_idx, end_idx):
            ...
            task = asyncio.create_task(run_one(orchestrator, scenario, True, semaphore, i))
            tasks.append(task)
            if i < end_idx - 1:
    ...
    logger.info("All 100 Spatial Runs Completed.")

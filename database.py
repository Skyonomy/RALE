...
# Cloud Run Production Safeguard
if os.getenv("K_SERVICE") and not os.getenv("POSTGRES_URL"):
    raise RuntimeError("POSTGRES_URL is required in Cloud Run production mode to prevent data fragmentation.")
...
class Run(Base):
    ...
    spatial_regime = Column(String, default="normal")
...
def log_run(run_id, scenario, spatial_regime="normal"):
    db = SessionLocal()
    try:
        run = db.query(Run).filter(Run.run_id == run_id).first()
        if not run:
            new_run = Run(run_id=run_id, scenario=scenario, status='STARTED', spatial_regime=spatial_regime)
            db.add(new_run)
            db.commit()
    finally:
        db.close()
